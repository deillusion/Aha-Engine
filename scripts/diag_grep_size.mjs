...
// 先看 data/runs 下超大单行文件的行结构
const files = (await host.listFiles('**/*', { maxResults: 5000 })).filter(f => f.startsWith('data/runs/'));
const lineShapes = [];
...
for (const query of useQueries) {
  for (const maxResults of [100, 2000]) {
    ...
    const fromRuns = matches.filter(m => m.file.startsWith('data/runs/')).length;
